...
export const getCacheKey = async (
  ...
  return {
    ...
    partial: [`setup-rust-${os.platform()}-${os.arch()}-`, 'setup-rust-'],
  }
}
...
export const main = async (inputsOverride?: Inputs, install = true) => {
  ...
  info(`Using toolchain "${toolchain.channel}" with profile "${toolchain.profile}"`)
  ...
  if (inputs.cache) {
    binaries.push('cargo-cache', 'cargo-sweep')
  }
  if (inputs.sccache) {
